...
class AutofillManagementStringBuilderImplTest {
    ...
    private val testee = AutofillManagementStringBuilderImpl(context = context, deviceSyncState = deviceSyncState)

    @Test
    fun whenDeletingOneLoginThenBuildsCorrectTitle() {
        val str = testee.stringForDeleteAllPasswordsDialogConfirmationDialogTitle(numberToDelete = 1)
        assertEquals(DELETE_DIALOG_TITLE_1_PASSWORD, str)
    }

    @Test
    fun whenDeletingMultipleLoginsThenBuildsCorrectTitle() {
        val str = testee.stringForDeleteAllPasswordsDialogConfirmationDialogTitle(numberToDelete = 2)
        assertEquals(DELETE_DIALOG_TITLE_2_PASSWORDS, str)
    }

    @Test
    fun whenDeletingOneLoginWithSyncEnabledThenBuildsCorrectMessage() {
        configureSyncState(enabled = true)
        val str = testee.stringForDeleteLoginsConfirmationDialogMessage(numberToDelete = 1)
        assertEquals("$DELETE_DIALOG_MESSAGE_1_SYNC_ENABLED_SINGULAR $DELETE_DIALOG_MESSAGE_2_SINGULAR", str)
    }

    @Test
    fun whenDeletingOneLoginWithSyncDisabledThenBuildsCorrectMessage() {
    ...
    @Test
    fun whenDeletingTwoLoginsWithSyncEnabledThenBuildsCorrectMessage() {
        configureSyncState(enabled = true)
        val str = testee.stringForDeleteLoginsConfirmationDialogMessage(numberToDelete = 2)
        assertEquals("$DELETE_DIALOG_MESSAGE_1_SYNC_ENABLED_PLURAL $DELETE_DIALOG_MESSAGE_2_PLURAL", str)
    }

    @Test
    fun whenDeletingTwoLoginsWithSyncDisabledThenBuildsCorrectMessage() {
        configureSyncState(enabled = false)
        val str = testee.stringForDeleteLoginsConfirmationDialogMessage(numberToDelete = 2)
